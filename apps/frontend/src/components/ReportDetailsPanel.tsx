...
export async function fetchReportDetails(reportId: string): Promise<ReportData> {
  // Simulate API delay
  ...
  const mockReports: { [key: string]: ReportData } = {
    'security-audit-2024': {
      ...
      summary: 'Comprehensive security assessment covering network infrastructure, application security, and compliance requirements.',
...
The security team will focus on application security improvements and staff training enhancements. A follow-up audit is scheduled for Q1 2025.`,
      ...
      attachments: ['security-audit-detailed.pdf', 'vulnerability-scan-results.xlsx'],
      metrics: {
        views: 127,
        downloads: 23,
        shares: 8
    ...
    'performance-analysis-2024': {
      ...
      summary: 'Detailed analysis of system performance metrics, bottlenecks, and optimization recommendations.',
...
export const ReportDetailsPanel = React.forwardRef<HTMLDivElement, ReportDetailsPanelProps>(({ 
  open,
  onClose,
  isDarkMode,
  reportData,
  ...
  const handleMouseUp = useCallback(() => {
    ...
    try {
      ...
      console.warn('Failed to save report panel width to localStorage:', error);
    }
  }, [panelWidth]);
  ...
  const sidebarContent = (
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', paddingRight: '48px' }}>
          <ReportIcon sx={{ 
            fontSize: 20, 
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            <Box sx={{ marginBottom: '16px' }}>
              ...
              <Box sx={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                <Chip
                  ...
                  size="small"
                  sx={{
                    backgroundColor: getStatusColor(reportData.status, theme) + '20',
                ...
                <Chip
                  ...
                  size="small"
                  sx={{
                    backgroundColor: getPriorityColor(reportData.priority, theme) + '20',
            ...
            <Box sx={{ 
              ...
              {reportData.createdDate && (
                <Typography variant="body2" sx={{
                  ...
                  <strong>Created:</strong> {new Date(reportData.createdDate).toLocaleDateString()}
                </Typography>
              )}
              {reportData.lastModified && (
                <Typography variant="body2" sx={{
                  ...
                  <strong>Last Modified:</strong> {new Date(reportData.lastModified).toLocaleDateString()}
                </Typography>
              )}
            </Box>
            ...
            {reportData.metrics && (
              <Box sx={{ marginBottom: '16px' }}>
                ...
                <Box sx={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <ShareIcon sx={{ fontSize: 16, color: theme.palette.text.secondary }} />
                    <Typography variant="body2" sx={{ fontSize: '13px' }}>
            ...
            {reportData.tags && reportData.tags.length > 0 && (
              <Box sx={{ marginTop: '24px', paddingTop: '16px', borderTop: `1px solid ${theme.palette.divider}` }}>
                <Typography variant="body2" sx={{
                  fontSize: '14px',
                  color: theme.palette.text.secondary,
                  marginBottom: '8px',
            ...
            {reportData.attachments && reportData.attachments.length > 0 && (
              <Box sx={{ marginTop: '16px', paddingTop: '16px', borderTop: `1px solid ${theme.palette.divider}` }}>
                <Typography variant="body2" sx={{
                  fontSize: '14px',
                  color: theme.palette.text.secondary,
                  marginBottom: '8px',
          ...
          <Typography variant="body2" sx={{ color: theme.palette.text.secondary, textAlign: 'center', marginTop: '40px' }}>
