# iAgent DevOps Project Setup Script (PowerShell)
# Complete infrastructure management for final project

param(
    [Parameter(Position=0)]
    [ValidateSet("start", "stop", "status", "deploy", "clean", "help")]
    [string]$Command = "help"
)
...
function Show-Status {
    ...
    try {
        aws cloudformation describe-stacks --stack-name $STACK_NAME --region $AWS_REGION 2>$null | Out-Null
    ...
    try {
        aws cloudformation describe-stacks --stack-name $MONITORING_STACK_NAME --region $AWS_REGION 2>$null | Out-Null
...
function Clean-All {
    ...
    Write-Status "Deleting ECR repositories..."
    try {
        aws ecr delete-repository --repository-name iagent-backend --force --region $AWS_REGION
    ...
    try {
        aws ecr delete-repository --repository-name iagent-frontend --force --region $AWS_REGION
        Write-Success "Frontend ECR repository deleted"
    }
    catch {
        Write-Warning "Failed to delete frontend ECR repository"
    }
    
    # Delete EKS cluster
    Write-Status "Deleting EKS cluster..."
    try {
        aws eks delete-cluster --name $CLUSTER_NAME --region $AWS_REGION
        Write-Success "EKS cluster deletion initiated"
    }
    catch {
        Write-Warning "Failed to delete EKS cluster"
    }
    
    # Delete CloudFormation stacks
    Write-Status "Deleting CloudFormation stacks..."
    try {
        aws cloudformation delete-stack --stack-name $MONITORING_STACK_NAME --region $AWS_REGION
        Write-Success "Monitoring stack deletion initiated"
    }
    catch {
        Write-Warning "Failed to delete monitoring stack"
    }
    
    try {
        aws cloudformation delete-stack --stack-name $STACK_NAME --region $AWS_REGION
        Write-Success "Infrastructure stack deletion initiated"
    }
    catch {
        Write-Warning "Failed to delete infrastructure stack"
    }
    
    Write-Success "Cleanup completed. Note: Some resources may take time to fully delete."
}

# Function to show help
function Show-Help {
    Write-Host "iAgent DevOps Project Setup Script (PowerShell)" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  deploy            Deploy applications only (infrastructure must be running)" -ForegroundColor Yellow
    Write-Host "  clean             Force cleanup all AWS resources" -ForegroundColor Yellow
...
function Main {
    switch ($Command) {
        ...
        "clean" {
            Write-Warning "This will permanently delete ALL AWS resources!"
